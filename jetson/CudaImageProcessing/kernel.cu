...
cudaError_t grayscaleFiler(unsigned char* resultBMP, const unsigned char* inputBMP, const int width, unsigned int height);

cudaError_t gaussianBlurFiler(unsigned char* resultBMP, const unsigned char* inputBMP, const int width, unsigned int height);
...
{
    // 1. Calculate the 2D coordinates of this thread
    int x = blockIdx.x * blockDim.x + threadIdx.x;
    int y = blockIdx.y * blockDim.y + threadIdx.y;

    // 2. Bounds check (ensure we stay inside the image)
    if (x < width && y < height) {

        // 3. Calculate the global 1D index for the start of this pixel
        // multiplied by 3 because each pixel has 3 bytes (B, G, R)
        int idx = (y * width + x) * 3;

        // 4. Read the BGR values (BMP stores BGR, not RGB)
        unsigned char b = input[idx];     // Blue
        unsigned char g = input[idx + 1]; // Green
        unsigned char r = input[idx + 2]; // Red

        // 5. Compute Grayscale (Luminosity Method is best for human eyes)
        // Formula: 0.21 R + 0.72 G + 0.07 B
        unsigned char gray = (unsigned char)(0.07f * b + 0.71f * g + 0.21f * r);

        // 6. Write the result (Set all 3 channels to 'gray')
        output[idx] = gray;
        output[idx + 1] = gray;
        output[idx + 2] = gray;
    }
...
int main()
{
    ...
    if (imageData) 
    {
        ...
        cudaStatus = grayscaleFiler(resultImage, imageData, width, height);
        if (cudaStatus != cudaSuccess) {
            fprintf(stderr, "grayscaleFiler failed!");
            return 1;
        }
		writeBMP("output.bmp", width, height, resultImage);
        delete[] imageData;
	}
